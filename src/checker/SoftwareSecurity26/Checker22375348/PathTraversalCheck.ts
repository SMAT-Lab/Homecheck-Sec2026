...
const gMetaData: BaseMetaData = {
    severity: 2,
    ruleDocPath: '',
    description: 'Detects path traversal vulnerability via file path manipulation.'
};
        ...
        for (const arkClass of targetFile.getClasses()) {
            for (const arkMethod of arkClass.getMethods()) {
                ...
                for (const stmt of cfg.getStmts()) {
                    ...
                    if ((text.includes('readfile') || text.includes('writefile') ||
                        text.includes('open') || text.includes('createfile') ||
                        text.includes('stat') || text.includes('fs.') ||
                        text.includes('file.') || text.includes('path.join')) &&
