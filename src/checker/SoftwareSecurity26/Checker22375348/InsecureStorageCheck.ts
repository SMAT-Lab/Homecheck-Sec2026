...
const logger = Logger.getLogger(LOG_MODULE_TYPE.HOMECHECK, 'InsecureStorageCheck');

const gMetaData: BaseMetaData = {
    severity: 2,
    ruleDocPath: '',
    description: 'Detects insecure storage of sensitive data in shared preferences or local storage.'
};

export class InsecureStorageCheck implements BaseChecker {
        ...
        for (const arkClass of targetFile.getClasses()) {
            for (const arkMethod of arkClass.getMethods()) {
                ...
                for (const stmt of cfg.getStmts()) {
                    ...
                    if ((text.includes('setstring') || text.includes('setint') ||
                        text.includes('setfloat') || text.includes('setboolean') ||
                        text.includes('setitem') || text.includes('localstorage') ||
                        text.includes('sharedpreferences') || text.includes('storage')) &&
                        (text.includes('password') || text.includes('secret') ||
                        text.includes('key') || text.includes('token') ||
                        text.includes('credential') || text.includes('auth'))) {
