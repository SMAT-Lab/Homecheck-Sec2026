...
const gMetaData: BaseMetaData = {
    severity: 2,
    ruleDocPath: '',
    description: 'Detects potential SQL injection vulnerabilities via string concatenation in SQL queries.'
};
        ...
        for (const arkClass of targetFile.getClasses()) {
            for (const arkMethod of arkClass.getMethods()) {
                ...
                for (const stmt of cfg.getStmts()) {
                    ...
                    if (text && text.includes('SELECT') && (text.includes("'") || text.includes('"'))) {
