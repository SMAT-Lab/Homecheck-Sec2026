...
const gMetaData: BaseMetaData = {
    severity: 1,
    ruleDocPath: '',
    description: 'Detects sensitive information written to logs.'
};

const SENSITIVE_KEYWORDS = ['password', 'pwd', 'secret', 'token', 'apikey', 'api_key', 'key'];
        ...
        for (const arkClass of targetFile.getClasses()) {
            for (const arkMethod of arkClass.getMethods()) {
                ...
                for (const stmt of cfg.getStmts()) {
                    const text = (stmt.getOriginalText() ?? '').toLowerCase();
                    if (text.includes('console.log') || text.includes('logger.log') || text.includes('console.error')) {
