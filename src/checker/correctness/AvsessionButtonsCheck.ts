...
const multimediaAvSessionSignList: string[] = [
...
const multimediaType: string = `@ohosSdk/api/@ohos.multimedia.avsession.d.ts: avSession.AVSession`;

const audioButtons: string[] = ['play', 'pause', 'stop', 'playNext', 'playPrevious'];
const avSessionType: string[] = ['audio', 'video'];
const multimediaButtonStmtList: Set<Stmt> = new Set();
...
export class AvsessionButtonsCheck implements BaseChecker {
    ...
    private processReportIssue(): void {
        ...
        for (let cmi of multimediaCreateList) {
            ...
            for (let buttonStmt of buttonStmts) {
                let invoke = CheckerUtils.getInvokeExprFromStmt(buttonStmt);
                if (!invoke) {
                    continue;
                }
    ...
    private processArkMethod(arkFile: ArkFile, method: ArkMethod): void {
        ...
        for (let i = 0; i < stmts.length; i++) {
            ...
            let avSessionInvoke = CheckerUtils.getInvokeExprFromStmt(stmt);
            if (!avSessionInvoke) {
                continue;
            }
            ...
            if (avSessionArg instanceof StringConstant && !avSessionType.includes(avSessionArg.getValue())) {
                continue;
            }
    ...
    private isInstanceAssignToVariable(callbackMethod: ArkMethod, stmt: ArkAssignStmt, useType: UseType): boolean {
        ...
        if (!(rightType instanceof UnknownType) && multimediaType === rightType.toString()) {
            return true;
        }
        let leftOp = stmt.getLeftOp();
