...
async function main(): Promise<void> {
    const sampleDir = path.join(__dirname, '../../sample/Sample23373456');
    ...
    const issueDirs: string[] = fs.readdirSync(sampleDir)
        .map((dir: string) => path.join(sampleDir, dir))
        .filter((dir: string) => fs.statSync(dir).isDirectory())
        .sort();
