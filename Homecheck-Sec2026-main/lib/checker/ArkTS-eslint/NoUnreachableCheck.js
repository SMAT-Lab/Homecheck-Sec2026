...
exports.NoUnreachableCheck = void 0;
const arkanalyzer_1 = require("arkanalyzer");
const Index_1 = require("../../Index");
const Index_2 = require("../../Index");
const DefectsList_1 = require("../../utils/common/DefectsList");
const Defects_1 = require("../../model/Defects");
class NoUnreachableCheck {
    rule;
    defects = [];
    issues = [];
    metaData = {
    ...
    // 预处理箭头函数，查找其中的不可达代码
    preprocessArrowFunctions(node, sourceFile, errors) {
        // 非箭头函数直接遍历子节点
        if (!arkanalyzer_1.ts.isArrowFunction(node)) {
            arkanalyzer_1.ts.forEachChild(node, child => this.preprocessArrowFunctions(child, sourceFile, errors));
            return;
        }
        // 处理箭头函数的主体
    ...
    // 处理箭头函数的主体
    processArrowFunctionBody(node, sourceFile, errors) {
        // 处理块语句形式的箭头函数体
        ...
        // 处理简写形式的箭头函数体
        if (arkanalyzer_1.ts.isConditionalExpression(node.body)) {
            this.preprocessConditionalExpression(node.body, sourceFile, errors);
        }
    }
    // 处理块语句形式的箭头函数体
    processArrowFunctionBlockBody(body, sourceFile, errors) {
        const statements = body.statements;
        for (let i = 0; i < statements.length; i++) {
            const currentStatement = statements[i];
            // 处理 switch 语句
            if (arkanalyzer_1.ts.isSwitchStatement(currentStatement)) {
    ...
    // 检查箭头函数中的 switch 语句
    checkSwitchStatementInArrowFunction(switchStmt, index, statements, sourceFile, errors) {
        if (!arkanalyzer_1.ts.isSwitchStatement(switchStmt)) {
            return;
        }
        // 检查 switch 语句是否有终止控制流的 default 分支
        const hasTerminatingDefault = this.hasSwitchTerminatingDefault(switchStmt);
    ...
    // 检查 switch 语句是否有终止控制流的 default 分支
    hasSwitchTerminatingDefault(switchStmt) {
        return switchStmt.caseBlock.clauses.some(clause => {
            return clause.kind === arkanalyzer_1.ts.SyntaxKind.DefaultClause &&
                clause.statements.some(stmt => arkanalyzer_1.ts.isThrowStatement(stmt) || arkanalyzer_1.ts.isReturnStatement(stmt));
        });
    }
    // 检查 switch 语句后的不可达语句
    checkUnreachableStatementAfterSwitch(nextStmt, sourceFile, errors) {
    ...
    // 判断语句是否是潜在的不可达语句
    isUnreachableStatementCandidate(stmt) {
        return arkanalyzer_1.ts.isEmptyStatement(stmt) ||
            (arkanalyzer_1.ts.isExpressionStatement(stmt) &&
                (stmt.expression.kind === arkanalyzer_1.ts.SyntaxKind.SemicolonToken ||
                    arkanalyzer_1.ts.isIdentifier(stmt.expression)));
    }
    // 检查控制流终止语句
    checkControlFlowTerminationInArrowFunction(currentStatement, index, statements, sourceFile, errors) {
        ...
        if (this.isVarDeclarationStmt(nextStmt) || arkanalyzer_1.ts.isFunctionDeclaration(nextStmt)) {
            return;
        }
        // 添加不可达代码的错误
        this.addUnreachableErrorForStatement(nextStmt, sourceFile, errors);
    ...
    // 为语句添加不可达错误
    addUnreachableErrorForStatement(stmt, sourceFile, errors) {
        this.addUnreachableError(stmt, sourceFile, errors);
    }
    visitAST(node, sourceFile, errors) {
        if (!node) {
            return;
        }
        this.processNodeByType(node, sourceFile, errors);
        arkanalyzer_1.ts.forEachChild(node, child => this.visitAST(child, sourceFile, errors));
    }
    processNodeByType(node, sourceFile, errors) {
        // 新增条件表达式检查
    ...
    isControlFlowEndingStatement(node) {
        return arkanalyzer_1.ts.isReturnStatement(node) ||
            arkanalyzer_1.ts.isThrowStatement(node) ||
            arkanalyzer_1.ts.isBreakStatement(node) ||
            arkanalyzer_1.ts.isContinueStatement(node);
    }
    checkOtherNodeTypes(node, sourceFile, errors) {
        if (arkanalyzer_1.ts.isForStatement(node) && !node.condition) {
        ...
        else if (arkanalyzer_1.ts.isSourceFile(node) && !arkanalyzer_1.ts.isClassDeclaration(node)) {
    ...
    isLoopNode(node) {
        return arkanalyzer_1.ts.isWhileStatement(node) ||
            arkanalyzer_1.ts.isForStatement(node) ||
            arkanalyzer_1.ts.isDoStatement(node);
    }
    checkInfiniteForLoop(node, sourceFile, errors) {
        const nextStatement = this.findNextStatement(node);
        if (nextStatement &&
            !arkanalyzer_1.ts.isFunctionDeclaration(nextStatement) &&
    ...
    isFunctionBodyTerminated(body) {
        if (arkanalyzer_1.ts.isBlock(body)) {
            return body.statements.some(stmt => arkanalyzer_1.ts.isReturnStatement(stmt));
        }
        // 处理箭头函数简写形式中的 return 语句
        return arkanalyzer_1.ts.isReturnStatement(body);
    }
    // 增强条件表达式检查逻辑
    checkConditionalExpression(expr, sourceFile, errors, ifStmt) {
        // 处理 IIFE 场景（立即执行函数）
        if (arkanalyzer_1.ts.isCallExpression(expr)) {
            const func = this.unwrapParentheses(expr.expression);
            if ((arkanalyzer_1.ts.isFunctionExpression(func) || arkanalyzer_1.ts.isArrowFunction(func)) &&
                this.isFunctionBodyTerminated(func.body)) {
                this.markIfBlockUnreachable(ifStmt.thenStatement, sourceFile, errors);
            }
        }
        // 处理函数声明作为条件的场景
        else if (arkanalyzer_1.ts.isFunctionExpression(expr) || arkanalyzer_1.ts.isArrowFunction(expr)) {
    ...
    unwrapParentheses(node) {
        while (arkanalyzer_1.ts.isParenthesizedExpression(node)) {
            node = node.expression;
        }
        return node;
    }
    // 精确标记代码块起始位置
    markIfBlockUnreachable(thenStmt, sourceFile, errors) {
        if (!arkanalyzer_1.ts.isBlock(thenStmt)) {
            return;
        }
        // 获取代码块起始大括号的位置
        const openBrace = thenStmt.getFirstToken();
        if (openBrace?.kind === arkanalyzer_1.ts.SyntaxKind.OpenBraceToken) {
            const { line, character } = sourceFile.getLineAndCharacterOfPosition(openBrace.getStart());
            errors.push({
                line: line + 1,
                character: character + 1,
                endCol: character + 1 + 1,
                message: 'Unreachable code'
            });
        }
    }
    addUnreachableCodeError(statement, node, sourceFile, errors) {
        this.addUnreachableError(statement, sourceFile, errors);
    }
    // 处理 if 语句
    checkIfStatement(node, sourceFile, errors) {
    ...
    // 检查 if 语句之后的代码
    checkNextStatementAfterIf(node, sourceFile, errors) {
        const nextStatement = this.findNextStatement(node);
        if (nextStatement && !arkanalyzer_1.ts.isFunctionDeclaration(nextStatement)) {
    ...
    // 添加不可达错误
    addUnreachableError(node, sourceFile, errors) {
    ...
    // 检查源文件中的语句
    checkSourceFileStatements(node, sourceFile, errors) {
        ...
        for (let i = 0; i < node.statements.length; i++) {
            ...
            // 处理已标记为不可达的代码
            if (isUnreachable) {
                const result = this.processUnreachableStatement(currentStatement, reportedFirstInSequence, afterClassDeclaration, sourceFile, errors);
    ...
    isOnlySwitchOrSimpleExpressions(node) {
        return node.statements.every(stmt => arkanalyzer_1.ts.isSwitchStatement(stmt) ||
            (arkanalyzer_1.ts.isExpressionStatement(stmt) && arkanalyzer_1.ts.isIdentifier(stmt.expression)) ||
            arkanalyzer_1.ts.isEmptyStatement(stmt));
    }
    // 检查是否应该跳过当前语句
    shouldSkipStatement(statement, index, node) {
        return arkanalyzer_1.ts.isSwitchStatement(statement) ||
            (index > 0 && arkanalyzer_1.ts.isSwitchStatement(node.statements[index - 1]));
    }
    // 检查是否为终止流程的语句
    isFlowTerminatingStatement(statement) {
        return this.isControlFlowTerminated(statement) && !arkanalyzer_1.ts.isSwitchStatement(statement);
    }
    // 处理已标记为不可达的语句
    processUnreachableStatement(statement, reportedFirstInSequence, afterClassDeclaration, sourceFile, errors) {
        // 函数声明在任何位置都是可达的
        ...
        // 已经报告过错误或空语句，无需再次报告
        if (reportedFirstInSequence || arkanalyzer_1.ts.isEmptyStatement(statement)) {
            return {
    ...
    // 检查是否为类声明后的特殊语句
    isStatementAfterClass(statement) {
        return arkanalyzer_1.ts.isVariableStatement(statement) || arkanalyzer_1.ts.isExpressionStatement(statement);
    }
    // 报告不可达语句错误
    reportUnreachableStatement(statement, sourceFile, errors) {
        this.addUnreachableError(statement, sourceFile, errors);
    }
    // 检查函数体
    checkFunctionBody(node, sourceFile, errors) {
    ...
    // 检查函数体中的语句
    checkFunctionStatements(body, sourceFile, errors) {
        for (let i = 0; i < body.statements.length; i++) {
            const currentStatement = body.statements[i];
            // 检查无限for循环中是否包含break语句
            if (arkanalyzer_1.ts.isForStatement(currentStatement) && !currentStatement.condition) {
    ...
    // 检查无限循环是否包含break语句
    infiniteForLoopHasBreak(node) {
        if (!arkanalyzer_1.ts.isBlock(node.statement)) {
            return arkanalyzer_1.ts.isBreakStatement(node.statement);
        }
        let hasBreak = false;
        function visit(node) {
            if (arkanalyzer_1.ts.isBreakStatement(node)) {
                hasBreak = true;
                return;
            }
            if (!hasBreak) {
        ...
        arkanalyzer_1.ts.forEachChild(node.statement, visit);
        return hasBreak;
    }
    // 检查函数中的下一条语句
    checkNextFunctionStatement(currentStatement, sourceFile, errors) {
        const nextStatement = this.findNextStatement(currentStatement);
        if (nextStatement &&
            !arkanalyzer_1.ts.isEmptyStatement(nextStatement) &&
            !arkanalyzer_1.ts.isFunctionDeclaration(nextStatement) &&
    ...
    // 处理循环语句
    checkLoopStatement(node, sourceFile, errors) {
        const body = arkanalyzer_1.ts.isForStatement(node) ? node.statement : node.statement;
        if (arkanalyzer_1.ts.isBlock(body)) {
            this.checkLoopBody(body, node, sourceFile, errors);
        }
    }
    // 检查循环体
    checkLoopBody(body, node, sourceFile, errors) {
        let isUnreachable = false;
    ...
    // 检查循环中不可达的语句
    checkUnreachableLoopStatement(statement, sourceFile, errors, node) {
        if (!this.isVarDeclarationStmt(statement)) {
            this.addUnreachableError(statement, sourceFile, errors);
        }
    }
    // 处理类声明
    checkClassDeclaration(node, sourceFile, errors) {
    ...
    checkClassExtends(node) {
        return node.heritageClauses?.some(clause => clause.token === arkanalyzer_1.ts.SyntaxKind.ExtendsKeyword) ?? false;
    }
    // 检查类的构造函数
    checkClassConstructor(classMembers, node, sourceFile, errors) {
        const constructorE = classMembers.find(member => arkanalyzer_1.ts.isConstructorDeclaration(member));
        if (constructorE) {
            // 检查构造函数是否调用了 super()
    ...
    // 检查不可达的实例字段
    findConstructorIndex(classMembers) {
        for (let i = 0; i < classMembers.length; i++) {
            if (arkanalyzer_1.ts.isConstructorDeclaration(classMembers[i])) {
                return i;
            }
        }
        return -1;
    ...
    isStaticProperty(member) {
        return arkanalyzer_1.ts.isPropertyDeclaration(member) &&
            !!member.modifiers?.some(mod => mod.kind === arkanalyzer_1.ts.SyntaxKind.StaticKeyword);
    }
    isInstanceProperty(member) {
        return arkanalyzer_1.ts.isPropertyDeclaration(member) &&
            !member.modifiers?.some(mod => mod.kind === arkanalyzer_1.ts.SyntaxKind.StaticKeyword);
    }
    reportUnreachableField(member, sourceFile, errors) {
        this.addUnreachableError(member, sourceFile, errors);
    }
    checkFieldsInRange(classMembers, startIndex, endIndex, sourceFile, errors) {
        let currentGroup = 0;
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            if (this.isInstanceProperty(member) && lastReportedGroup !== currentGroup) {
                this.reportUnreachableField(member, sourceFile, errors);
                lastReportedGroup = currentGroup;
            }
        }
    }
    checkUnreachableInstanceFields(classMembers, node, sourceFile, errors) {
    ...
    checkTryStatement(node, sourceFile, errors) {
    ...
    isInAsyncFunction(node) {
        ...
        while (current) {
            if ((arkanalyzer_1.ts.isFunctionDeclaration(current) ||
                ...
                arkanalyzer_1.ts.isMethodDeclaration(current)) &&
                current.modifiers?.some(mod => mod.kind === arkanalyzer_1.ts.SyntaxKind.AsyncKeyword)) {
                return true;
            }
    ...
    // 检查是否只包含 throw 语句
    hasOnlyThrowStatement(block) {
        let hasThrow = false;
        let hasOtherTerminator = false;
        const checkStatement = (statement) => {
            if (arkanalyzer_1.ts.isThrowStatement(statement)) {
                hasThrow = true;
            }
            else if (this.isControlFlowTerminated(statement) && !arkanalyzer_1.ts.isThrowStatement(statement)) {
                hasOtherTerminator = true;
            }
        };
        // 检查块中的每个语句
        for (const statement of block.statements) {
            if (arkanalyzer_1.ts.isBlock(statement)) {
                // 如果是嵌套块，递归检查
    ...
    // 检查 catch 块的可达性
    checkCatchBlockReachability(catchClause, node, sourceFile, errors) {
        if (catchClause && catchClause.block) {
            this.addUnreachableError(catchClause.block, sourceFile, errors);
        }
    }
    // 检查 try 语句之后的代码
    checkNextStatementAfterTry(node, sourceFile, errors) {
        const nextStatement = this.findNextStatement(node);
        if (nextStatement && !arkanalyzer_1.ts.isFunctionDeclaration(nextStatement)) {
            this.addUnreachableError(nextStatement, sourceFile, errors);
        }
    ...
    // 检查 finally 块
    checkFinallyBlock(finallyBlock, node, sourceFile, errors) {
    ...
    checkSwitchStatement(node, sourceFile, errors) {
        ...
        for (const caseClause of node.caseBlock.clauses) {
            ...
            for (const statement of caseClause.statements) {
                if (this.isControlFlowTerminated(statement) && !arkanalyzer_1.ts.isBreakStatement(statement)) {
                    hasTerminator = true;
                    break;
                }
            }
        ...
        if (allCasesTerminateFlow) {
            const nextStatement = this.findNextStatement(node);
            if (nextStatement && !arkanalyzer_1.ts.isEmptyStatement(nextStatement) && !arkanalyzer_1.ts.isFunctionDeclaration(nextStatement)) {
                this.addUnreachableError(nextStatement, sourceFile, errors);
            }
        }
    }
    // 判断是否为 var 声明
    isVarDeclarationStmt(node) {
        if (arkanalyzer_1.ts.isVariableStatement(node)) {
            ...
            let flag = (node.declarationList.flags & arkanalyzer_1.ts.NodeFlags.Let) === 0 &&
                (node.declarationList.flags & arkanalyzer_1.ts.NodeFlags.Const) === 0 &&
    ...
    findNextStatement(node) {
        ...
        if (arkanalyzer_1.ts.isBlock(parent)) {
            ...
            if (index < parent.statements.length - 1) {
                ...
                if (arkanalyzer_1.ts.isEmptyStatement(nextStmt) && index + 2 < parent.statements.length) {
    ...
    // 检查基本的控制流终止语句
    isBasicControlFlowTerminator(node) {
    ...
    // 检查复杂的控制流终止语句
    isComplexControlFlowTerminator(node) {
        // 处理 ForStatement
        ...
        // 处理 try-finally 语句
        if (arkanalyzer_1.ts.isTryStatement(node)) {
        ...
        if (arkanalyzer_1.ts.isWhileStatement(node)) {
            if (node.expression.kind === arkanalyzer_1.ts.SyntaxKind.TrueKeyword) {
    ...
    // 检查 case 子句的终止情况
    checkCaseClauseTermination(caseClause) {
        for (const statement of caseClause.statements) {
            if (this.isControlFlowTerminated(statement)) {
                return true;
            }
    ...
    // 检查无限 while 循环的代码块
    checkInfiniteWhileLoopBlock(block) {
        for (const statement of block.statements) {
            if (this.isControlFlowTerminated(statement)) {
                return false;
            }
    ...
    // 检查构造函数中是否调用了 super(),.../
    hasSuperCallInConstructor(constructorEle) {
        let hasSuperCall = false;
        function visit(node) {
            if (arkanalyzer_1.ts.isCallExpression(node) && node.expression.kind === arkanalyzer_1.ts.SyntaxKind.SuperKeyword) {
                hasSuperCall = true;
            }
            arkanalyzer_1.ts.forEachChild(node, visit);
        }
        if (constructorEle) {
            arkanalyzer_1.ts.forEachChild(constructorEle, visit);
        }
        return hasSuperCall;
    }
    // 添加辅助判断函数
    isBreakOrContinue(node) {
        return arkanalyzer_1.ts.isBreakStatement(node) || arkanalyzer_1.ts.isContinueStatement(node);
    }
    // 专门检查不可达的分号，特别是针对箭头函数中的 switch 语句后的分号
    checkUnreachableSemicolons(node, sourceFile, errors) {
        // 非箭头函数直接遍历子节点
        if (!arkanalyzer_1.ts.isArrowFunction(node)) {
            arkanalyzer_1.ts.forEachChild(node, child => this.checkUnreachableSemicolons(child, sourceFile, errors));
            return;
        }
        // 非块语句形式的箭头函数体直接遍历子节点
        if (!arkanalyzer_1.ts.isBlock(node.body)) {
    ...
    // 处理箭头函数体内的不可达分号
    checkUnreachableSemicolonsInArrowFunction(body, sourceFile, errors) {
        const statements = body.statements;
        ...
        for (let i = 0; i < statements.length - 1; i++) {
            ...
            if (!arkanalyzer_1.ts.isSwitchStatement(currentStmt) || !arkanalyzer_1.ts.isEmptyStatement(nextStmt)) {
                continue;
            }
            this.checkSwitchWithTerminatingDefault(currentStmt, nextStmt, sourceFile, errors);
        }
    }
    // 检查 switch 语句是否有终止控制流的 default 分支，如果有则标记后续分号为不可达
    checkSwitchWithTerminatingDefault(switchStmt, nextStmt, sourceFile, errors) {
        // 查找 default 分支
    ...
    findDefaultClause(switchStmt) {
        return switchStmt.caseBlock.clauses.find(clause => clause.kind === arkanalyzer_1.ts.SyntaxKind.DefaultClause);
    }
    // 检查 case/default 分支是否含有终止控制流的语句
    hasTerminatingStatement(clause) {
        return clause.statements.some(stmt => arkanalyzer_1.ts.isThrowStatement(stmt) || arkanalyzer_1.ts.isReturnStatement(stmt));
    }
    // 标记语句为不可达代码
    markStatementAsUnreachable(stmt, sourceFile, errors) {
        this.addUnreachableError(stmt, sourceFile, errors);
    }
    // 处理三元表达式中的不可达代码
    preprocessConditionalExpression(node, sourceFile, errors) {
        // 检查冒号后面的表达式是否终止控制流
        if (!arkanalyzer_1.ts.isCallExpression(node.whenFalse)) {
            return;
        }
        const callExpr = node.whenFalse;
        // 检查是否调用了会抛出异常的函数
        if (!arkanalyzer_1.ts.isIdentifier(callExpr.expression) ||
        ...
        // 检查父节点后面是否有分号或其他语句
        const parent = node.parent;
        if (!arkanalyzer_1.ts.isExpressionStatement(parent) || !parent.parent || !arkanalyzer_1.ts.isBlock(parent.parent)) {
            return;
        }
        ...
        if (!arkanalyzer_1.ts.isEmptyStatement(nextStmt)) {
            return;
        }
        // 添加不可达代码的错误
        this.addUnreachableErrorForStatement(nextStmt, sourceFile, errors);
    ...
    isExportStatement(node) {
        ...
        // 检查是否为导出声明
        if (arkanalyzer_1.ts.isExportDeclaration(node)) {
            return true;
        }
        // 检查是否为带有export修饰符的声明
        if (arkanalyzer_1.ts.isVariableStatement(node) ||
            arkanalyzer_1.ts.isFunctionDeclaration(node) ||
            arkanalyzer_1.ts.isClassDeclaration(node)) {
            return node.modifiers?.some(mod => mod.kind === arkanalyzer_1.ts.SyntaxKind.ExportKeyword) ?? false;
        }
        // 检查是否为默认导出
        if (arkanalyzer_1.ts.isExportAssignment(node)) {
            return true;
        }
        return false;
    }
}
exports.NoUnreachableCheck = NoUnreachableCheck;
