...
exports.NoRegexSpacesCheck = void 0;
const arkanalyzer_1 = require("arkanalyzer");
const Index_1 = require("../../Index");
const DefectsList_1 = require("../../utils/common/DefectsList");
const Defects_1 = require("../../model/Defects");
;
class NoRegexSpacesCheck {
    defects = [];
    issues = [];
    rule;
    metaData = {
    ...
    ;
    check = (target) => {
        const astRoot = arkanalyzer_1.AstTreeUtils.getSourceFileFromArkFile(target);
    ...
    checkAction(astRoot, sourceFile) {
        const violations = [];
        const checkNode = (node) => {
            ...
            if ((arkanalyzer_1.ts.isNewExpression(node) || arkanalyzer_1.ts.isCallExpression(node)) &&
    ...
    ;
    checkRegexLiteral(node, sourceFile, violations) {
    ...
    ;
    hasInvalidMultipleSpaces(regexText) {
        let state = {
            inCharClass: false,
            inQuantifier: false,
            inLookahead: false,
    ...
    ;
    handleCharClass(state, char, regexText, index) {
        if (char === '[' && !state.escaped) {
    ...
    ;
    handleLookahead(state, char, regexText, index) {
    ...
    ;
    handleEscapedCharacter(state, char) {
        if (state.escaped) {
            state.escaped = false;
            if (char === ' ') {
                state.spaceCount = 0;
    ...
    ;
    isInvalidSpace(state, regexText, index) {
        // 在任何层级的字符类中都允许连续空格
        if (state.charClassDepth > 0) {
            return false;
        }
    ...
    ;
    checkRegExpConstructor(node, sourceFile, violations, parentNode) {
        if (node.arguments && node.arguments.length > 0 && arkanalyzer_1.ts.isStringLiteral(node.arguments[0])) {
    ...
    fixLookaheads(text) {
        return text.replace(/\(\?=([^)]*)\)/g, (match, inside) => {
            // 修复前瞻断言内的空格
            const fixedInside = inside.replace(/ {2,}/g, (spaces) => {
    ...
    fixQuantifiers(text) {
        // 特殊处理量词序列情况：如 "  {3}" 或 "  +" 等
        return text.replace(this.spacesRegex, (spaces) => {
            return ` {${spaces.length - 1}} `;
        });
    }
    fixRegularSpaces(text) {
        // 修复其他普通连续空格
        return text.replace(/ {2,}/g, (spaces) => {
            return ` {${spaces.length}}`;
        });
    }
    restoreSavedParts(text, savedParts) {
     ...
     * 处理转义字符
     */
    handleEscapeCharacter(text, index, currentSpaces, maxSpaces) {
        // 跳过转义字符和被转义的字符
        index += 2;
        // 不立即重置计数器，先检查是否需要记录
     ...
     * 处理字符类
     */
    handleCharacterClass(text, index, currentSpaces, maxSpaces) {
        // 首先检查当前累积的空格数
        maxSpaces = this.updateMaxSpaceCount(currentSpaces, maxSpaces);
        // 跳过整个字符类内容
     ...
     * 处理非空格字符
     */
    handleNonSpaceCharacter(char, currentSpaces, maxSpaces) {
        // 遇到非空格字符，处理已累积的空格
        if (currentSpaces >= 2) {
            // 检查是否紧跟量词
     ...
     * 更新最大空格计数
     */
    updateMaxSpaceCount(currentSpaces, maxSpaces) {
    ...
    ;
    // 新增：获取第一个连续空格序列的长度
    getFirstSpaceSequenceLength(text) {
        // 跳过开始的'/'分隔符
    ...
    addIssueReport(violation, filePath) {
        ...
        let defect = new Index_1.Defects(violation.line, violation.character, violation.character, this.metaData.description, severity, this.rule.ruleId, filePath, this.metaData.ruleDocPath, true, false, true);
        this.defects.push(defect);
        DefectsList_1.RuleListUtil.push(defect);
        return defect;
    }
    ;
    findClosingBracket(text, start) {
        let i = start + 1;
        while (i < text.length) {
            if (text[i] === '\\') {
                // 跳过转义字符
...
exports.NoRegexSpacesCheck = NoRegexSpacesCheck;
