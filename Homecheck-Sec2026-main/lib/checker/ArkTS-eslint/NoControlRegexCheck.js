...
exports.NoControlRegexCheck = void 0;
...
const logger = logger_1.default.getLogger(logger_1.LOG_MODULE_TYPE.HOMECHECK, 'NoControlRegexCheck');
class NoControlRegexCheck {
    REG_EXP = 'RegExp';
    rule;
    defects = [];
    issues = [];
    ...
    check = (target) => {
        ...
        for (const stmt of stmts) {
            ...
            if (className === this.REG_EXP) {
                ...
                if (arkInvokeStmts[0] && arkInvokeStmts[0] instanceof Constant_1.StringConstant) {
            ...
            if (result) {
                if (rightOp instanceof Constant_1.Constant) {
    ...
    getCheck(stmt, rightOp) {
        if (rightOp instanceof arkanalyzer_1.ArkStaticInvokeExpr) {
            ...
            if (regex) {
                ...
                if (args[0] && args[0] instanceof Constant_1.StringConstant) {
    ...
    addIssueReport(stmt, sign, message) {
        ...
        const defect = new Defects_1.Defects(warnInfo.line, warnInfo.startCol, warnInfo.endCol, currentDescription, severity, this.rule.ruleId, warnInfo.filePath, this.metaData.ruleDocPath, true, false, false);
        this.issues.push(new Defects_1.IssueReport(defect, undefined));
        DefectsList_1.RuleListUtil.push(defect);
    }
    getLineAndColumn(stmt, sign) {
...
exports.NoControlRegexCheck = NoControlRegexCheck;
