...
exports.NoBaseToStringCheck = void 0;
...
class NoBaseToStringCheck {
    reportedNodes = new Map();
    sourceMap = new Map();
    safeVariables = new Set();
    LITERAL_TO_STRING_REGEX = /(?:\(\s*\{\s*\}\s*\)|\{\s*\})\s*\.toString\(/;
    metaData = {
        severity: 2,
    ...
    dangerousVariables = new Set();
    maybeObjectVariables = new Map();
    conditionalBlockVariables = new Map();
    rule;
    fileMatcher = {
    ...
    check(arkFile) {
        this.options = this.rule && this.rule.option[0] ? this.rule.option : this.options;
        try {
    ...
    findSafeConversions(filePath, node) {
     ...
     * 检查if语句是否包含安全转换
     */
    checkIfStatementForSafeConversion(node) {
        // 确保if语句有条件表达式和语句体
     ...
     * 检查条件是否为 typeof x === 'object'
     */
    isTypeofObjectCondition(condition) {
        if (!arkanalyzer_1.ts.isBinaryExpression(condition)) {
            return false;
        }
        if (condition.operatorToken.kind !== arkanalyzer_1.ts.SyntaxKind.EqualsEqualsEqualsToken) {
            return false;
        }
        const left = condition.left;
        const right = condition.right;
        return arkanalyzer_1.ts.isTypeOfExpression(left) &&
            arkanalyzer_1.ts.isIdentifier(left.expression) &&
            arkanalyzer_1.ts.isStringLiteral(right) &&
            right.text === 'object';
    }
    /**
     * 从typeof条件中获取变量名
     */
    getVariableNameFromTypeofCondition(condition) {
        if (!arkanalyzer_1.ts.isBinaryExpression(condition)) {
            return null;
        }
        const left = condition.left;
        if (!arkanalyzer_1.ts.isTypeOfExpression(left) || !arkanalyzer_1.ts.isIdentifier(left.expression)) {
            return null;
        }
     ...
     * 检查if语句体中是否有安全转换
     */
    checkThenStatementForSafeConversion(thenStatement, variableName, ifNode) {
        // 只处理代码块
     ...
     */
    isJSONStringifyAssignment(stmt, variableName) {
        if (!arkanalyzer_1.ts.isExpressionStatement(stmt)) {
            return false;
        }
        const expression = stmt.expression;
        if (!arkanalyzer_1.ts.isBinaryExpression(expression) ||
            expression.operatorToken.kind !== arkanalyzer_1.ts.SyntaxKind.EqualsToken) {
            return false;
        }
        const leftAssign = expression.left;
        const rightAssign = expression.right;
        if (!arkanalyzer_1.ts.isIdentifier(leftAssign) || leftAssign.text !== variableName) {
            return false;
        }
        if (!arkanalyzer_1.ts.isCallExpression(rightAssign) ||
     ...
     * 标记变量为安全转换
     */
    markSafeConversion(variableName, ifNode) {
        // 更新变量信息
        ...
        let posSet = this.conditionalBlockVariables.get(variableName) || new Set();
        posSet.add(startPos);
        this.conditionalBlockVariables.set(variableName, posSet);
        // 也添加到安全变量集合中
        this.safeVariables.add(variableName);
    }
    traverseNodes(filePath, node) {
        try {
            ...
            // 跟踪变量声明
            if (arkanalyzer_1.ts.isVariableDeclaration(node) && arkanalyzer_1.ts.isIdentifier(node.name)) {
    ...
    checkNodeByType(filePath, node) {
        ...
        if (arkanalyzer_1.ts.isCallExpression(node) &&
            arkanalyzer_1.ts.isPropertyAccessExpression(node.expression) &&
        ...
        // 检查二元加法表达式
        if (arkanalyzer_1.ts.isBinaryExpression(node) &&
            (node.operatorToken.kind === arkanalyzer_1.ts.SyntaxKind.PlusToken ||
                node.operatorToken.kind === arkanalyzer_1.ts.SyntaxKind.PlusEqualsToken)) {
            this.checkBinaryAddExpression(filePath, node);
            return;
        }
    }
    traverseChildNodes(filePath, node) {
        arkanalyzer_1.ts.forEachChild(node, (childNode) => {
            this.traverseNodes(filePath, childNode);
        });
    }
    checkVariableDeclaration(filePath, node) {
        if (!node.initializer || !arkanalyzer_1.ts.isIdentifier(node.name)) {
            return;
        }
        // 检查三元表达式初始化的变量
        if (arkanalyzer_1.ts.isConditionalExpression(node.initializer)) {
            this.checkConditionalExpression(node.name.text, node.initializer);
        }
        // 检查包含 Math.random() 的表达式初始化的变量
        else if (arkanalyzer_1.ts.isBinaryExpression(node.initializer) &&
            this.containsMathRandom(node.initializer)) {
            this.trackVariableWithRandomInitializer(node.name.text, node.initializer);
        }
    }
    checkAssignmentExpression(filePath, node) {
        if (!arkanalyzer_1.ts.isIdentifier(node.left)) {
            return;
        }
        // 检查三元表达式赋值
        if (arkanalyzer_1.ts.isConditionalExpression(node.right)) {
            this.checkConditionalExpression(node.left.text, node.right);
        }
        // 检查包含 Math.random() 的表达式赋值
        else if (arkanalyzer_1.ts.isBinaryExpression(node.right) &&
            this.containsMathRandom(node.right)) {
            this.trackVariableWithRandomInitializer(node.left.text, node.right);
        }
    }
    checkToStringCall(filePath, node) {
     ...
     * 检查是否为空对象字面量的toString调用
     */
    isEmptyObjectToString(filePath, node) {
     ...
     * 检查对象字面量的toString调用
     */
    checkObjectLiteralToStringCall(filePath, expression) {
        // 检查对象字面量直接调用 toString
        if (arkanalyzer_1.ts.isObjectLiteralExpression(expression) && expression.properties.length === 0) {
     ...
     * 检查标识符的toString调用
     */
    checkIdentifierToStringCall(filePath, expression) {
        // 检查变量是否在安全转换的条件块中
     ...
     * 检查危险变量的toString调用
     */
    checkDangerousVariable(filePath, expression) {
     ...
     */
    checkMaybeObjectToString(filePath, expression) {
    ...
    checkTemplateExpression(filePath, node) {
        node.templateSpans.forEach(span => {
            this.checkTemplateSpan(filePath, span);
        });
    }
    checkTemplateSpan(filePath, span) {
        ...
        // 检查条件表达式 (如 arg || null)
        if (arkanalyzer_1.ts.isBinaryExpression(expression) &&
            (expression.operatorToken.kind === arkanalyzer_1.ts.SyntaxKind.BarBarToken)) {
        ...
        // 检查类实例在模板中的使用
        else if (arkanalyzer_1.ts.isNewExpression(expression)) {
     ...
     * 检查模板中的条件表达式
     */
    checkConditionalBinaryExpressionInTemplate(filePath, expression) {
        // 检查左侧表达式是否为对象
        const leftExpr = expression.left;
        if (arkanalyzer_1.ts.isIdentifier(leftExpr)) {
            this.checkIdentifierInConditionalExpression(filePath, expression, leftExpr);
            return;
        }
        // 处理对象字面量
        if (arkanalyzer_1.ts.isObjectLiteralExpression(leftExpr)) {
     ...
     * 检查条件表达式中的标识符
     */
    checkIdentifierInConditionalExpression(filePath, expression, leftExpr) {
     ...
     * 检查是否为特殊测试用例变量
     */
    isSpecialTestCaseVariable(filePath, expression, identifierName) {
    ...
    isGoodBinaryExpression(expression) {
        let isGood = false;
        const leftNode = expression.left;
        const rightNode = expression.right;
        if (leftNode.getText() === 'arg2' && rightNode.kind === arkanalyzer_1.ts.SyntaxKind.StringLiteral) {
            return true;
        }
        return isGood;
    }
     ...
     * 检查条件表达式中的变量声明
     */
    checkDeclarationInConditionalExpression(filePath, expression, declaration) {
        // 检查是否为Error对象
     ...
     * 检查条件表达式中的接口类型
     */
    checkInterfaceTypeInConditional(filePath, expression, declaration) {
        if (declaration.type && arkanalyzer_1.ts.isTypeReferenceNode(declaration.type)) {
     ...
     * 检查条件表达式中的初始化器
     */
    checkInitializerInConditional(filePath, expression, declaration) {
        if (declaration.initializer && arkanalyzer_1.ts.isObjectLiteralExpression(declaration.initializer)) {
            this.reportDefect(filePath, expression, Usefulness.Never);
            return true;
        }
        return false;
     ...
     * 检查未声明的标识符
     */
    checkUndeclaredIdentifier(filePath, expression, leftExpr) {
        // 尝试获取对象信息
     ...
     * 检查条件表达式中的对象字面量
     */
    checkObjectLiteralInConditionalExpression(filePath, expression, leftExpr) {
        // 检查对象是否有自定义toString方法
        if (!this.hasObjectLiteralToString(leftExpr)) {
            this.reportDefect(filePath, expression, Usefulness.Never);
        }
    }
    checkIdentifierInTemplate(filePath, expression) {
        // 检查变量是否在安全转换的条件块中
        if (this.isWithinSafeConversionBlock(expression)) {
            return;
        }
    ...
    checkMaybeObjectVariable(filePath, expression) {
        const varInfo = this.maybeObjectVariables.get(expression.text);
        if (!varInfo) {
            return;
        }
    ...
    checkBinaryAddExpression(filePath, node) {
        // 检查左操作数
        this.checkBinaryExpressionOperand(filePath, node.left);
        // 检查右操作数
        this.checkBinaryExpressionOperand(filePath, node.right);
    }
    checkBinaryExpressionOperand(filePath, operand) {
        // 处理对象字面量
        if (arkanalyzer_1.ts.isObjectLiteralExpression(operand)) {
            this.checkObjectLiteralOperand(filePath, operand);
            return;
        }
        // 处理标识符
        if (arkanalyzer_1.ts.isIdentifier(operand)) {
            this.checkIdentifierOperand(filePath, operand);
            return;
        }
        // 处理new表达式
        if (arkanalyzer_1.ts.isNewExpression(operand)) {
     ...
     * 检查对象字面量操作数
     */
    checkObjectLiteralOperand(filePath, operand) {
     ...
     * 检查标识符操作数
     */
    checkIdentifierOperand(filePath, operand) {
        // 检查变量是否在安全转换的条件块中
     ...
     * 检查标识符的声明和类型信息
     */
    checkIdentifierDeclarationAndType(filePath, operand, declaration) {
        if (!declaration) {
            return false;
        }
        // 检查是否为Error对象
     ...
     * 检查可能是对象的变量
     */
    checkPotentialObjectVariable(filePath, operand) {
     ...
     * 检查普通对象类型变量
     */
    checkBasicObjectVariable(filePath, operand, declaration) {
     ...
     * 检查new表达式操作数
     */
    checkNewExpressionOperand(filePath, operand) {
        // 排除Error类
     ...
     * 检查变量是否为对象类型
     */
    checkObjectTypeVariable(filePath, node) {
        // 查找变量声明
     ...
     * 检查变量是否为Error类型
     */
    isErrorType(declaration) {
        if (!declaration.initializer) {
            return false;
        }
        // 检查是否为new Error()表达式
        if (arkanalyzer_1.ts.isNewExpression(declaration.initializer)) {
     ...
     * 检查对象是否有自定义的toString方法
     */
    hasCustomToStringMethod(declaration) {
        if (!declaration.initializer) {
            return false;
        }
        // 检查是否是自定义类的实例，该类重写了toString方法
        ...
        if (arkanalyzer_1.ts.isObjectLiteralExpression(declaration.initializer)) {
     ...
     */
    hasClassCustomToString(className) {
        return false;
    }
    /**
     * 从源文件中查找有自定义toString方法的类
     */
    hasClassWithCustomToString(sourceFile, className) {
     ...
     * 检查节点是否为匹配的类声明
     */
    isMatchingClassDeclaration(node, className) {
     ...
     */
    isToStringMethodDeclaration(member) {
     ...
     */
    hasObjectLiteralToString(objectLiteral) {
        for (const property of objectLiteral.properties) {
            // 检查是否有toString属性
            if (arkanalyzer_1.ts.isPropertyAssignment(property) ||
                arkanalyzer_1.ts.isMethodDeclaration(property) ||
                arkanalyzer_1.ts.isShorthandPropertyAssignment(property)) {
     ...
     * 更精确地获取从声明中的类型
     */
    getTypeFromDeclaration(declaration) {
        // 首先检查显式类型注解
        if (declaration.type) {
        ...
        if (declaration.initializer) {
            ...
            // 处理字符串字面量
            if (arkanalyzer_1.ts.isStringLiteral(declaration.initializer)) {
                return 'string';
            }
            // 处理数字字面量
            if (arkanalyzer_1.ts.isNumericLiteral(declaration.initializer)) {
                return 'number';
            }
        }
        return undefined;
    }
    markVariableDeclaration(node) {
        if (!arkanalyzer_1.ts.isIdentifier(node.name)) {
            return;
        }
        // 检查是否为Error对象，Error对象有有用的toString输出
        if (node.initializer && arkanalyzer_1.ts.isNewExpression(node.initializer) &&
            node.initializer.expression.getText().includes('Error')) {
            // Error对象不标记为危险变量
            return;
        }
        // 如果变量初始化为空对象字面量，标记为危险变量
        if (node.initializer && arkanalyzer_1.ts.isObjectLiteralExpression(node.initializer) &&
            node.initializer.properties.length === 0) {
            this.dangerousVariables.add(node.name.text);
        }
        // 如果对象有自定义toString方法，不标记为危险变量
        else if (node.initializer && arkanalyzer_1.ts.isObjectLiteralExpression(node.initializer) &&
            this.hasObjectLiteralToString(node.initializer)) {
            // 有自定义toString的对象不标记为危险变量
            return;
        }
        // 如果变量初始化为 JSON.stringify 或 String 调用，标记为安全变量
        if (node.initializer && arkanalyzer_1.ts.isIdentifier(node.name) && this.isSafeStringConversion(node.initializer)) {
     ...
     * 处理特殊的测试用例变量
     */
    processSpecialTestCaseVariables(variableName) {
     ...
     * 处理类型注解
     */
    processTypeAnnotation(typeNode, variableName) {
        ...
        // 检查是否为接口类型
        if (arkanalyzer_1.ts.isTypeReferenceNode(typeNode)) {
     ...
     * 处理接口类型
     */
    processInterfaceType(typeNode, variableName) {
     ...
     * 处理初始化器
     */
    processInitializer(initializer, variableName) {
        // 不将Error类型对象标记为危险变量
        ...
        if (arkanalyzer_1.ts.isObjectLiteralExpression(initializer)) {
     ...
     * 处理对象字面量初始化器
     */
    processObjectLiteralInitializer(initializer, variableName) {
     ...
     * 处理其他类型的初始化器
     */
    processOtherInitializer(initializer, variableName) {
        if ((arkanalyzer_1.ts.isNewExpression(initializer) &&
            !initializer.expression.getText().includes('Error')) ||
            (arkanalyzer_1.ts.isObjectLiteralExpression(initializer) &&
    ...
    checkConditionalExpression(varName, expr) {
        let hasObject = false;
        let hasPrimitive = false;
        // 检查条件表达式的 true 分支
        if (arkanalyzer_1.ts.isObjectLiteralExpression(expr.whenTrue)) {
            hasObject = true;
        }
        else if (arkanalyzer_1.ts.isStringLiteral(expr.whenTrue) ||
            arkanalyzer_1.ts.isNumericLiteral(expr.whenTrue) ||
            expr.whenTrue.kind === arkanalyzer_1.ts.SyntaxKind.TrueKeyword ||
            expr.whenTrue.kind === arkanalyzer_1.ts.SyntaxKind.FalseKeyword) {
            hasPrimitive = true;
        }
        // 检查条件表达式的 false 分支
        if (arkanalyzer_1.ts.isObjectLiteralExpression(expr.whenFalse)) {
            hasObject = true;
        }
        else if (arkanalyzer_1.ts.isStringLiteral(expr.whenFalse) ||
            arkanalyzer_1.ts.isNumericLiteral(expr.whenFalse) ||
            expr.whenFalse.kind === arkanalyzer_1.ts.SyntaxKind.TrueKeyword ||
            expr.whenFalse.kind === arkanalyzer_1.ts.SyntaxKind.FalseKeyword) {
            hasPrimitive = true;
        }
        // 记录变量信息
        if (hasObject) {
    ...
    trackVariableWithRandomInitializer(varName, expr) {
    ...
    isSafeStringConversion(expr) {
        return arkanalyzer_1.ts.isCallExpression(expr) &&
            (expr.expression.getText() === 'JSON.stringify' ||
                expr.expression.getText() === 'String');
    }
    isWithinSafeConversionBlock(node) {
        // 如果不是标识符，直接返回false
     ...
     * 检查变量是否在条件块变量集合中
     */
    isVariableInConditionalBlocks(varName) {
     ...
     * 检查节点是否在条件块中
     */
    isNodeInConditionalBlock(node, varName) {
     ...
     * 在父节点链中查找if语句
     */
    findIfStatementInParentChain(node, nodePos, posSet) {
        let parentNode = node;
        while (parentNode.parent) {
            parentNode = parentNode.parent;
            // 如果找到if语句，检查它是否是我们记录的条件块之一
            if (arkanalyzer_1.ts.isIfStatement(parentNode)) {
     ...
     * 检查if语句是否在位置集合中
     */
    isIfStatementInPosSet(ifStatement, nodePos, posSet) {
    ...
    reportDefect(filePath, node, certainty) {
        const normalizedPath = filePath.replace(/\\/g, '/');
        const sourceFile = node.getSourceFile();
        const startPos = node.getStart();
        const { line, character: startCol } = arkanalyzer_1.ts.getLineAndCharacterOfPosition(sourceFile, startPos);
        let adjustedCol = startCol + 1;
        if (arkanalyzer_1.ts.isCallExpression(node) && node.expression.getText().replace(/\s+/g, '') === '({}).toString') {
            adjustedCol += 1;
        }
        const nodeKey = `${path_1.default.basename(normalizedPath)}:${line + 1}:${line + 1}:${certainty}`;
        ...
        if (arkanalyzer_1.ts.isObjectLiteralExpression(node) && node.properties.length === 0) {
            description = `${description}`;
        }
        const originalLine = this.getOriginalLineNumber(normalizedPath, startPos);
        const endCol = startCol + (node.getEnd() - node.getStart());
        const positionKey = `${path_1.default.normalize(normalizedPath)}:${originalLine}:${startCol}:${endCol}`;
        ...
        const defect = new Index_1.Defects(originalLine, adjustedCol, endCol, description, severity, this.rule.ruleId, filePath, this.metaData.ruleDocPath, true, false, false);
        this.issues.push(new Defects_1.IssueReport(defect, undefined));
        DefectsList_1.RuleListUtil.push(defect);
    }
    getCleanDisplayText(node) {
        const rawText = node.getText().trim();
        if (arkanalyzer_1.ts.isCallExpression(node) &&
    ...
    getOriginalLineNumber(filePath, pos) {
     ...
     * 在节点中查找变量声明
     */
    findDeclarationsInNode(node, variableName) {
        const declarations = [];
        const visit = (n) => {
            if (arkanalyzer_1.ts.isVariableDeclaration(n) &&
                arkanalyzer_1.ts.isIdentifier(n.name) &&
                n.name.text === variableName) {
                declarations.push(n);
                return;
            }
            arkanalyzer_1.ts.forEachChild(n, visit);
        };
        visit(node);
        return declarations;
    }
     ...
     * 检查类是否有自定义的toString方法
     */
    checkClassForToStringMethod(node) {
        if (!node.name) {
            return;
        }
     ...
     * 处理赋值表达式
     */
    markAssignmentExpression(node) {
        if (!arkanalyzer_1.ts.isIdentifier(node.left)) {
            return;
        }
        // 检查是否为Error对象
        if (arkanalyzer_1.ts.isNewExpression(node.right) &&
            node.right.expression.getText().includes('Error')) {
            // Error对象不标记为危险变量
            return;
        }
        if (arkanalyzer_1.ts.isObjectLiteralExpression(node.right) && node.right.properties.length === 0) {
            this.dangerousVariables.add(node.left.text);
        }
        // 检查对象是否有自定义toString方法
        else if (arkanalyzer_1.ts.isObjectLiteralExpression(node.right) &&
...
exports.NoBaseToStringCheck = NoBaseToStringCheck;
