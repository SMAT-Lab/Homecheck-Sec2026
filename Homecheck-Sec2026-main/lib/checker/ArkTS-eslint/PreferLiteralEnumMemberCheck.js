...
exports.PreferLiteralEnumMemberCheck = void 0;
...
const bitwiseOperationReg = /^\s*\d+(\.\d+)?\s*(&|\||\^|~|<<|>>|>>>)+\s*\d+(\.\d+)?\s*$/;
...
//推荐显式初始化每个枚举成员值。
class PreferLiteralEnumMemberCheck {
    metaData = gMetaData;
    rule;
    defects = [];
    issues = [];
     ...
     */
    isPureStringOrNumberOrStartsEndsWithSlash(input, allowBitwise) {
        if (allowBitwise) {
            // 检查输入字符串是否匹配任一正则表达式
            return (pureStringRegex.test(input) ||
     ...
     */
    isBitwiseOperationWithNumbers(input) {
    ...
    process(stmts, severity, mergedOptions) {
        for (const stmt of stmts) {
            ...
            if (stmt instanceof lib_1.ArkAssignStmt) {
                ...
                //判定是不是对象(特殊处理)放掉
                if (rightOPTYPE.classSignature?.className &&
                ...
                //判定是否合规
                if (!(rightOPSTR === 'null') &&
                    !this.isPureStringOrNumberOrStartsEndsWithSlash(rightOPSTR, mergedOptions.allowBitwiseExpressions ?? false)) {
                    this.addIssueReport(stmt, IssueCode, severity);
                    break; //禁止多余检索
                }
            }
...
exports.PreferLiteralEnumMemberCheck = PreferLiteralEnumMemberCheck;
