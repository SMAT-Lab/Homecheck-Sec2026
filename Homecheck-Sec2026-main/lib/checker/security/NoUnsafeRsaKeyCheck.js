...
exports.NoUnsafeRsaKeyCheck = void 0;
...
const logger = logger_1.default.getLogger(logger_1.LOG_MODULE_TYPE.HOMECHECK, 'NoUnsafeRsaKeyCheck');
const gMetaData = {
    severity: 2,
    ruleDocPath: 'docs/no-unsafe-rsa-key-check.md',
    description: 'Do not use insecure RSA keys. For example, the modules length of RSA keys is less than 2048 bits.'
};
const createAsyKeyGeneratorSignature = `@ohosSdk/api/@ohos.security.cryptoFramework.d.ts: cryptoFramework.${lib_1.DEFAULT_ARK_CLASS_NAME}.createAsyKeyGenerator(string)`;
class NoUnsafeRsaKeyCheck {
    metaData = gMetaData;
    rule;
    defects = [];
    issues = [];
    ...
    processStmt(stmts, arkClass) {
        for (let stmt of stmts) {
            ...
            if (signatureStr !== createAsyKeyGeneratorSignature) {
                continue;
            }
            ...
            if (parseStr.includes('RSA') && rsaNum < 2048) {
...
exports.NoUnsafeRsaKeyCheck = NoUnsafeRsaKeyCheck;
