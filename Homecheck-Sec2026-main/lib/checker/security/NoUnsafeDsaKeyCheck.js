...
exports.NoUnsafeDsaKeyCheck = void 0;
...
const logger = logger_1.default.getLogger(logger_1.LOG_MODULE_TYPE.HOMECHECK, 'NoUnsafeDsaKeyCheck');
const gMetaData = {
    severity: 2,
    ruleDocPath: 'docs/no-unsafe-dsa-key-check.md',
    description: 'Do not use insecure DSA keys, such as those with a DH modulus length less than 2048 bits.'
};
const creatCipherSignature = `@ohosSdk/api/@ohos.security.cryptoFramework.d.ts: cryptoFramework.${lib_1.DEFAULT_ARK_CLASS_NAME}.createAsyKeyGenerator(string)`;
class NoUnsafeDsaKeyCheck {
    metaData = gMetaData;
    rule;
    defects = [];
    issues = [];
    ...
    processStmt(stmts, arkClass) {
        for (let stmt of stmts) {
            ...
            if (signatureStr !== creatCipherSignature) {
                continue;
            }
            ...
            const dsaKey = parseStr.match(/\d+(\d+)?/g);
            if (!dsaKey) {
                continue;
            }
            const dsaKeyNum = parseInt(dsaKey[0]);
            if (parseStr.includes('DSA') && dsaKeyNum < 2048) {
...
exports.NoUnsafeDsaKeyCheck = NoUnsafeDsaKeyCheck;
