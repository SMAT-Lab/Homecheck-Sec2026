...
exports.NoUnsafeHashCheck = void 0;
...
const logger = logger_1.default.getLogger(logger_1.LOG_MODULE_TYPE.HOMECHECK, 'NoUnsafeHashCheck');
const gMetaData = {
    severity: 2,
    ruleDocPath: 'docs/no-unsafe-hash-check.md',
    description: 'DO not use insecure hash algorithms, such as MD5 and SHA1.'
};
const createMdSignature = `@ohosSdk/api/@ohos.security.cryptoFramework.d.ts: cryptoFramework.${lib_1.DEFAULT_ARK_CLASS_NAME}.createMd(string)`;
class NoUnsafeHashCheck {
    metaData = gMetaData;
    rule;
    defects = [];
    issues = [];
    ...
    processStmt(stmts, arkClass) {
        for (let stmt of stmts) {
            ...
            if (signatureStr !== createMdSignature) {
                continue;
            }
            ...
            if (parseStr.includes('SHA1') || parseStr.includes('MD5')) {
...
exports.NoUnsafeHashCheck = NoUnsafeHashCheck;
