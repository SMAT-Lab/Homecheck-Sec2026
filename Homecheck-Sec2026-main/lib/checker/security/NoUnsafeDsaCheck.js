...
exports.NoUnsafeDsaCheck = void 0;
...
const logger = logger_1.default.getLogger(logger_1.LOG_MODULE_TYPE.HOMECHECK, 'NoUnsafeDsaCheck');
const gMetaData = {
    severity: 2,
    ruleDocPath: 'docs/no-unsafe-dsa-check.md',
    description: 'Do not use insecure DSA, such as the situations where the DSA modulus length is less than 2048 bits or the insecure SHA1 hash algorithm is applied in the digest.'
};
const createSignature = [
    `@ohosSdk/api/@ohos.security.cryptoFramework.d.ts: cryptoFramework.${lib_1.DEFAULT_ARK_CLASS_NAME}.createSign(string)`,
    `@ohosSdk/api/@ohos.security.cryptoFramework.d.ts: cryptoFramework.${lib_1.DEFAULT_ARK_CLASS_NAME}.createVerify(string)`
];
class NoUnsafeDsaCheck {
    metaData = gMetaData;
    rule;
    defects = [];
    issues = [];
    ...
    processStmt(stmts, arkClass) {
        for (let stmt of stmts) {
            ...
            if (!createSignature.includes(signatureStr)) {
                continue;
            }
            ...
            const dsa = parseStr.match(/\d+(\d+)?/g);
            if (!dsa) {
                continue;
            }
            const dsaNum = parseInt(dsa[0]);
            if (parseStr.includes('DSA') && (dsaNum < 2048 || parseStr.includes('SHA1'))) {
...
exports.NoUnsafeDsaCheck = NoUnsafeDsaCheck;
