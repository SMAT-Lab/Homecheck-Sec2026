...
exports.NoUnsafeAesCheck = void 0;
...
const logger = logger_1.default.getLogger(logger_1.LOG_MODULE_TYPE.HOMECHECK, 'NoUnsafeAesCheck');
const gMetaData = {
    severity: 3,
    ruleDocPath: 'docs/no-unsafe-aes-check.md',
    description: 'Do not use the insecure ECB encryption mode in the AES encryption algorithm; it is recommended to use the secure AES interface in the Petal Aegis SDK.'
};
const creatCipherSignature = `@ohosSdk/api/@ohos.security.cryptoFramework.d.ts: cryptoFramework.${lib_1.DEFAULT_ARK_CLASS_NAME}.createCipher(string)`;
class NoUnsafeAesCheck {
    metaData = gMetaData;
    rule;
    defects = [];
    issues = [];
    ...
    processStmt(stmts, arkClass) {
        for (let stmt of stmts) {
            ...
            if (parseStr.includes('AES') && parseStr.includes('ECB')) {
...
exports.NoUnsafeAesCheck = NoUnsafeAesCheck;
