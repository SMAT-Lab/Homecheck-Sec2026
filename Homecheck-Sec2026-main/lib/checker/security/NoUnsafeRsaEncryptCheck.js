...
exports.NoUnsafeRsaEncryptCheck = void 0;
...
const logger = logger_1.default.getLogger(logger_1.LOG_MODULE_TYPE.HOMECHECK, 'NoUnsafeRsaEncryptCheck');
const gMetaData = {
    severity: 2,
    ruleDocPath: 'docs/no-unsafe-rsa-encrypt-check.md',
    description: 'DO not use insecure RSA asymmetric encryption algorithms. For example, the RSA modules length is less than 2048 bits, the padding mode is PKCS1, and the digest or mask digest uses insecure MD5 or SHA1 hash algorithms.'
};
const createCipherSignature = `@ohosSdk/api/@ohos.security.cryptoFramework.d.ts: cryptoFramework.${lib_1.DEFAULT_ARK_CLASS_NAME}.createCipher(string)`;
class NoUnsafeRsaEncryptCheck {
    metaData = gMetaData;
    rule;
    defects = [];
    issues = [];
    ...
    processStmt(stmts, arkClass) {
        for (let stmt of stmts) {
            ...
            if (signatureStr !== createCipherSignature) {
                continue;
            }
            ...
            let fillMode = parseStr.split('|')[1];
            if (parseStr.includes('RSA') && (rsaNum < 2048 || fillMode === 'PKCS1' || parseStr.includes('MD5') || parseStr.includes('SHA1'))) {
...
exports.NoUnsafeRsaEncryptCheck = NoUnsafeRsaEncryptCheck;
