...
exports.NoUnsafeDhCheck = void 0;
...
const logger = logger_1.default.getLogger(logger_1.LOG_MODULE_TYPE.HOMECHECK, 'NoUnsafeDhCheck');
const gMetaData = {
    severity: 2,
    ruleDocPath: 'docs/no-unsafe-dh-check.md',
    description: 'Do not use insecure DH key negotiation algorithms, such as those with a DH modulus length of less than 2048 bits.'
};
const creatCipherSignature = `@ohosSdk/api/@ohos.security.cryptoFramework.d.ts: cryptoFramework.${lib_1.DEFAULT_ARK_CLASS_NAME}.createKeyAgreement(string)`;
class NoUnsafeDhCheck {
    metaData = gMetaData;
    rule;
    defects = [];
    issues = [];
    ...
    processStmt(stmts, arkClass) {
        for (let stmt of stmts) {
            ...
            const dhNum = parseInt(parseStr.substring(parseStr.length - 4, parseStr.length + 1));
            if (parseStr.includes('DH') && dhNum < 2048) {
...
exports.NoUnsafeDhCheck = NoUnsafeDhCheck;
