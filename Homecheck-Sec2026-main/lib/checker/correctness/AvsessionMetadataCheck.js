...
exports.AvsessionMetadataCheck = void 0;
const arkanalyzer_1 = require("arkanalyzer");
const Index_1 = require("../../Index");
const logger_1 = __importStar(require("arkanalyzer/lib/utils/logger"));
const Defects_1 = require("../../model/Defects");
const multimediaAPI10CreateSignList = [
...
const multimediaStmtList = new Set();
const multimediaCreateList = [];
const logger = logger_1.default.getLogger(logger_1.LOG_MODULE_TYPE.HOMECHECK, 'AvsessionMetadataCheck');
const SETAVMETADATA = 'setAVMetadata';
const SETAVPLAYBACKSTATE = 'setAVPlaybackState';
const fieldInfo = ['mediaImage', 'title', 'author', 'subtitle', 'duration', 'state', 'position'];
const gMetaData = {
    severity: 1,
...
class AvsessionMetadataCheck {
    ...
    getMultipleScenariosField(stmt) {
    ...
    getField(stmt, arg, name) {
        let newFieldList = new Set();
        // 场景一：方法内部
    ...
    traversalStaticFields(clazz, arg, name) {
        let newFieldList = new Set();
        let declaringStmt = arg.getDeclaringStmt();
        if (!(declaringStmt instanceof arkanalyzer_1.ArkAssignStmt)) {
            return newFieldList;
        ...
        if (rightOp instanceof arkanalyzer_1.ArkStaticFieldRef) {
            ...
            if (!(declareSignature instanceof arkanalyzer_1.ClassSignature)) {
                return newFieldList;
            }
    ...
    isArkFieldGetFields(clazz, field) {
        let newFieldList = new Set();
    ...
    isHasAuthorAndSubtitle(newFieldList) {
    ...
    handleInvoker(callbackMethod, stmt, createInfo, busyMethods, useType) {
        let invoker = Index_1.CheckerUtils.getInvokeExprFromStmt(stmt);
        if (!invoker) {
            return;
        }
    ...
    getInvokeCallbackMethod(arkFile, stmt, methodName, invoker) {
        let arg = null;
    ...
    setStmt(methodName, createInfo, stmt) {
        if (methodName === SETAVMETADATA) {
            createInfo.avMetadataStmt = stmt;
        }
        else {
...
exports.AvsessionMetadataCheck = AvsessionMetadataCheck;
