...
exports.AvsessionButtonsCheck = void 0;
...
const multimediaAPICreateSignList = [
    `@ohosSdk/api/@ohos.multimedia.avsession.d.ts: avSession.${arkanalyzer_1.DEFAULT_ARK_CLASS_NAME}.createAVSession(@ohosSdk/api/application/BaseContext.d.ts: BaseContext, string, @ohosSdk/api/@ohos.multimedia.avsession.d.ts: avSession.${arkanalyzer_1.DEFAULT_ARK_CLASS_NAME}.[static]${arkanalyzer_1.DEFAULT_ARK_CLASS_NAME}()#AVSessionType)`,
    `@ohosSdk/api/@ohos.multimedia.avsession.d.ts: avSession.${arkanalyzer_1.DEFAULT_ARK_CLASS_NAME}.createAVSession(Context, string, @ohosSdk/api/@ohos.multimedia.avsession.d.ts: avSession.${arkanalyzer_1.DEFAULT_ARK_CLASS_NAME}.[static]${arkanalyzer_1.DEFAULT_ARK_CLASS_NAME}()#AVSessionType)`,
    `@ohosSdk/api/@ohos.multimedia.avsession.d.ts: avSession.${arkanalyzer_1.DEFAULT_ARK_CLASS_NAME}.createAVSession(@ohosSdk/api/application/BaseContext.d.ts: BaseContext, string, @ohosSdk/api/@ohos.multimedia.avsession.d.ts: avSession.${arkanalyzer_1.DEFAULT_ARK_CLASS_NAME}.[static]${arkanalyzer_1.DEFAULT_ARK_CLASS_NAME}()#AVSessionType, @ohosSdk/api/@ohos.base.d.ts: AsyncCallback<@ohosSdk/api/@ohos.multimedia.avsession.d.ts: avSession.AVSession,void>)`,
    `@ohosSdk/api/@ohos.multimedia.avsession.d.ts: avSession.${arkanalyzer_1.DEFAULT_ARK_CLASS_NAME}.createAVSession(Context, string, @ohosSdk/api/@ohos.multimedia.avsession.d.ts: avSession.${arkanalyzer_1.DEFAULT_ARK_CLASS_NAME}.[static]${arkanalyzer_1.DEFAULT_ARK_CLASS_NAME}()#AVSessionType, @ohosSdk/api/@ohos.base.d.ts: AsyncCallback<@ohosSdk/api/@ohos.multimedia.avsession.d.ts: avSession.AVSession,void>)`
];
const multimediaAvSessionSignList = [
    `@ohosSdk/api/@ohos.multimedia.avsession.d.ts: avSession.AVSession.on(string, @ohosSDK/api/@ohos.multimedia.avsession.d.ts: avSession.AVSession.%AM0())`,
    `@ohosSdk/api/@ohos.multimedia.avsession.d.ts: avSession.AVSession.off(string, @ohosSDK/api/@ohos.multimedia.avsession.d.ts: avSession.AVSession.%AM7())`,
];
const multimediaType = `@ohosSdk/api/@ohos.multimedia.avsession.d.ts: avSession.AVSession`;
const audioButtons = ['play', 'pause', 'stop', 'playNext', 'playPrevious'];
const avSessionType = ['audio', 'video'];
const multimediaButtonStmtList = new Set();
const multimediaCreateList = [];
const logger = logger_1.default.getLogger(logger_1.LOG_MODULE_TYPE.HOMECHECK, 'AvsessionButtonsCheck');
const gMetaData = {
    severity: 2,
...
(function (UseType) {
    ...
    UseType[UseType["PROMISE_AWAIT"] = 2] = "PROMISE_AWAIT";
    UseType[UseType["UNKNOWN"] = 3] = "UNKNOWN";
})(UseType || (UseType = {}));
class AvsessionButtonsCheck {
    metaData = gMetaData;
    rule;
    defects = [];
    issues = [];
    ...
    processReportIssue() {
        ...
        for (let cmi of multimediaCreateList) {
            ...
            for (let buttonStmt of buttonStmts) {
                let invoke = Index_1.CheckerUtils.getInvokeExprFromStmt(buttonStmt);
                if (!invoke) {
                    continue;
                }
                let arg = invoke.getArg(0);
                if (arg instanceof arkanalyzer_1.Constant) {
                    argSet.add(arg.toString());
                }
            }
            if (argSet.size !== 5) {
    ...
    processArkMethod(arkFile, method) {
        ...
        for (let i = 0; i < stmts.length; i++) {
            ...
            if (this.isAudioInterruptStmt(arkFile, stmt, invoker)) {
                multimediaButtonStmtList.add(stmt);
                continue;
            }
            if (!this.isMultimediaCreateStmt(stmt)) {
                continue;
            }
            let avSessionInvoke = Index_1.CheckerUtils.getInvokeExprFromStmt(stmt);
            if (!avSessionInvoke) {
                continue;
            }
            let avSessionArg = avSessionInvoke.getArg(2);
            if (!avSessionArg) {
                continue;
            }
            if (avSessionArg instanceof Constant_1.StringConstant && !avSessionType.includes(avSessionArg.getValue())) {
                continue;
            }
            let createInfo = {
            ...
            if (stmt instanceof arkanalyzer_1.ArkAssignStmt) {
                ...
                if (this.isAwaitReturnStmt(stmt)) {
                    this.processCallbackMethod(arkFile, method, i, createInfo, busyMethods, UseType.PROMISE_AWAIT);
                }
    ...
    processCallbackMethod(arkFile, callbackMethod, index, createInfo, busyMethods, useType) {
        ...
        for (let i = index; i < stmts.length; i++) {
            ...
            if (stmt instanceof arkanalyzer_1.ArkInvokeStmt && this.isResolveAssignToVariable(arkFile, stmt, useType)) {
    ...
    handleInvokeStmt(stmt, createInfo) {
    ...
    handleMultimediaArgs(arkFile, callbackMethod, invoker, createInfo, busyMethods, useType) {
    ...
    isArgTypeMultimedia(callbackMethod, arg, useType) {
        ...
        while (declaringStmt && backTrackCount < 12) {
            ...
            if (declaringStmt instanceof arkanalyzer_1.ArkAssignStmt) {
                ...
                if (multimediaType === leftOpType || multimediaType === rightOpType) {
                    return true;
                }
    ...
    commonInvokerMatch() {
        for (let cmi of multimediaCreateList) {
            let interruptInfoOn = cmi.interruptInfo;
            if (interruptInfoOn.size !== 0) {
                continue;
            }
            for (let stmt of multimediaButtonStmtList) {
                ...
                if (this.isInvokerAndStmtMatch(cmi, base)) {
                    interruptInfoOn.add(stmt);
                }
            }
            multimediaButtonStmtList.clear();
        }
    }
    ...
    isInstanceAssignToVariable(callbackMethod, stmt, useType) {
        ...
        if (useType === UseType.PROMISE_AWAIT) {
            if (this.isAwaitReturnStmt(stmt) && this.isMultimediaCreateStmt(stmt)) {
                return true;
            }
        }
        if (useType !== UseType.CALLBACK && useType !== UseType.PROMISE_THEN) {
            if (multimediaType === rightType.toString()) {
                return true;
            }
        }
        if (!(rightOp instanceof arkanalyzer_1.Local)) {
        ...
        if (!(rightType instanceof arkanalyzer_1.UnknownType) && multimediaType === rightType.toString()) {
            return true;
        }
        let leftOp = stmt.getLeftOp();
        ...
        if (parameterType instanceof arkanalyzer_1.UnknownType && rightOp instanceof arkanalyzer_1.UnknownType && (leftOp instanceof arkanalyzer_1.AbstractFieldRef)) {
            if (!(leftType instanceof arkanalyzer_1.UnionType)) {
                return multimediaType === leftType.toString();
            }
    ...
    isMultimediaCreateStmt(stmt) {
        ...
        if (apiVersion > 9) {
            return multimediaAPICreateSignList.includes(methodSignatureStr);
        }
        return false;
    }
...
exports.AvsessionButtonsCheck = AvsessionButtonsCheck;
