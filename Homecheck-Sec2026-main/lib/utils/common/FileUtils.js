...
exports.WriteFileMode = exports.FileUtils = void 0;
/*
...
const logger = logger_1.default.getLogger(logger_1.LOG_MODULE_TYPE.HOMECHECK, 'FileUtils');
class FileUtils {
    /**
     * 读取指定文件并返回其内容
     ...
     */
    static async getFiltedFiles(fileList, ruleConfig) {
        try {
     ...
     * @returns 符合条件的文件路径列表
     */
    static async matchFiles(fileList, fileGlob, ignoreGlob) {
        let result = [];
     ...
     */
    static async readLinesFromFile(filePath, lineNo) {
     ...
     * @returns 符合条件的文件信息数组
     */
    static getSeletctedFileInfos(jsonPath, exts) {
        const fileInfoList = [];
        try {
            const jsonData = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
            jsonData.checkPath?.forEach((fileInfo) => {
     ...
     * @returns 符合条件的文件路径数组
     */
    static getAllFiles(dirPath, exts, filenameArr = [], visited = new Set()) {
        // 检查目录是否存在
    ...
    static shouldAddFile(filePath, exts) {
        if (exts.length === 0) {
            return true;
        }
     ...
     **/
    static writeToFile(filePath, content, mode = WriteFileMode.APPEND) {
...
var WriteFileMode;
(function (WriteFileMode) {
    WriteFileMode[WriteFileMode["OVERWRITE"] = 0] = "OVERWRITE";
    WriteFileMode[WriteFileMode["APPEND"] = 1] = "APPEND";
})(WriteFileMode = exports.WriteFileMode || (exports.WriteFileMode = {}));
