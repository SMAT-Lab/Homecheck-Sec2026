...
import { DotNotationCheck } from "../../../src/checker/ArkTS-eslint/DotNotationCheck";
...
beforeAll(async () => {
  ...
  checkEntry = await testCaseCheck(
    "./test/unittest/sample/DotNotation",
    rule,
...
describe("DotNotationCheckTest", () => {
  /**
   * @tc.number:NoUnusedVarsCheckTest
   * @tc.name: 方法参数强制设定为只读默认不设置参数，上报
   * @tc.desc: 方法参数强制设定为只读默认不设置参数，上报
   */
  test("DotNotationCheckTest_001", () => {
    const detectFile: string = path.join(realPath, "ets", "default.ts");
   const expectReportList = ['19%4%', '21%7%', '22%13%', '23%13%', '24%5%','25%5%', '26%5%', '27%5%', '28%5%', '29%4%', '30%5%'];
   ...
   */
  test("DotNotationCheckTest_002", () => {
    const detectFile: string = path.join(realPath, "ets", "pass.ts");
