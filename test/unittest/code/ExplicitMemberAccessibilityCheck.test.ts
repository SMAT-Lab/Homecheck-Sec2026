...
import { ExplicitMemberAccessibilityCheck } from "../../../src/checker/ArkTS-eslint/ExplicitMemberAccessibilityCheck";
...
beforeAll(async () => {
  ...
  checkEntry = await testCaseCheck(
    "./test/unittest/sample/ExplicitMemberAccessibility",
    rule,
    CHECK_MODE.FILE2CHECK,
    ExplicitMemberAccessibilityCheck
  );
  realPath = checkEntry.scene.getRealProjectDir();
});

describe("ExplicitMemberAccessibilityCheckTest", () => {
  /**
   * @tc.number:NoUnusedVarsCheckTest
   * @tc.name: 方法参数强制设定为只读默认不设置参数，上报
   * @tc.desc: 方法参数强制设定为只读默认不设置参数，上报
   */
  test("ExplicitMemberAccessibilityCheckTest_001", () => {
    const detectFile: string = path.join(realPath, "ets", "default.ets");
   const expectReportList = ['16%3%', '20%3%', '21%3%', '25%3%', '29%3%'];
   ...
   */
  test("ExplicitMemberAccessibilityCheckTest_002", () => {
